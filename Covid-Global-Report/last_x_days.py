...
# This function creates a scatter chart for the given parameters
...
# This function creates a dataframe of the daily cases for the past x days for the given parameters
def lastx_days_data_country(last_days=10,country='India',mode=Mode.CASES,type='Cases',trend=20):
    df = load_relevant_data(mode=mode).groupby('Country/Region').sum(numeric_only=True).reset_index()
    ...
    plot_line(data=count,x=count.index,y='World',title=f'World {type}',xlable=None,ylable=type,filename=f'tmp/last_{last_days}_world_{type}.png',trend=trend)
    plot_line(data=count,x=count.index,y=country,title=f'{country} {type}',xlable=None,ylable=type,filename=f'tmp/last_{last_days}_{country}_{type}.png',trend=trend)
